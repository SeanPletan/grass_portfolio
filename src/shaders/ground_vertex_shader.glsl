...
float getElevation(vec3 position){
     ...
     for(int i=1;i<=4;i++)
     {
          elevation+=(amp*snoise(vec2(position.x,position.y+150.)*freq));
          amp*=gain;
          freq*=lac;
     }
     return elevation+length(vec2(position.x,position.y)*.02);
}

void main(){
     vUv=uv;
     
     vec4 localSpacePosition=vec4(position,1.);
     vec4 worldPosition=modelMatrix*localSpacePosition;
     worldPosition.y=getElevation(position);
